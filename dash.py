# ==============================================================================
# SCRIPT : CRYPTO DASHBOARD PRO - V20.28b - INTÉGRATION AUTO-GÉRÉE
# DATE : 2026-03-12
# PORT : 8080 - VERSION FINALE ET INTÉGRALE
# ==============================================================================
...
# --- LOGIQUE DYNAMIQUE AUTO-GÉRÉE ---
def get_macro_dates_intern():
    now = datetime.now()
    fed_date = datetime(2026, 3, 18)
    cpi_date = datetime(2026, 3, 17)
    
    def format_date(target):
        delta = (target - now).days
        if delta < 0: return None
        if delta == 0: return target.strftime("%d/%m") + " (jr J)"
        return target.strftime("%d/%m") + f" (J-{delta})"

    fed_fmt = format_date(fed_date)
    cpi_fmt = format_date(cpi_date)
    show_date = (int(time.time()) % 6) >= 3
    
    return {
        "fed": fed_fmt if (show_date and fed_fmt) else None, 
        "cpi": cpi_fmt if (show_date and cpi_fmt) else None
    }
    ...
    try:
        ...
        for key, url in urls.items():
            ...
            if res.status_code == 200:
                ...
                if content: final_blocks.append(" • ".join(content))
        if final_blocks: return "ACTUS : " + " ℹ️ ".join(final_blocks) + " ℹ️ "
...
def macro_api():
    ...
    return jsonify({
        ...
        "date_fed": dates["fed"], "date_cpi": dates["cpi"]
    })
...
<head>
    ...
    <style>
        ...
        .scroller-wrap { width: 100%; height: 24px; overflow: hidden; position: relative; }
        #wrap-n1 { margin-bottom: 2px; margin-top: 10px; }
        #wrap-n2 { margin-bottom: 20px; }
        .scroller-text { position: absolute; white-space: nowrap; font-size: 12px; font-weight: 500; left: 0; }
        ...
        .blink-u { border-bottom: 1px solid var(--mauve); animation: blink-border-only 1s step-end infinite; padding-bottom: 1px; color: var(--mauve) !important; }
        .macro-card { background: var(--block-grey); border: 1px solid var(--ui-grey); border-radius: 35px; padding: 18px 12px; }
...
<body>
    ...
    <script>
        ...
        function animate() {
            ...
            if(el2 && el2.offsetWidth > 0) { pos2 -= 1.0; if (Math.abs(pos2) >= el2.offsetWidth / 2) pos2 = 0; el2.style.transform = `translateX(${pos2}px)`; }
        ...
        function draw(candles) {
            ...
            if(isCandle) {
                ctx.fillStyle = "rgba(191,119,255,0.16)"; ctx.beginPath(); tech.forEach((t,i) => i===0 ? ctx.moveTo(getX(i),getY(t.bU)) : ctx.lineTo(getX(i),getY(t.bU))); for(let i=59; i>=0; i--) ctx.lineTo(getX(i),getY(t.bL)); ctx.fill();
            ...
            if (lastAge < 10 && lastSlope > 0 && globalLiqDelta > 0) app.className = "app active-g";
            else if (lastAge < 10 && lastSlope < 0 && Math.abs(lastSlope) > 0.05 && globalLiqDelta < 0) app.className = "app active-r";
            else if (lastAge < 10 && lastSlope < 0) app.className = "app active-r-soft";
        ...
        async function loadMacro() {
            try {
                ...
                document.getElementById('n1').innerText = d.news_top + d.news_top; document.getElementById('n2').innerHTML = d.events + d.events;
                ...
                document.getElementById('macro-content').innerHTML = `
                    ...
                    <div class="m-row"><span class="m-lbl">TAUX FED</span><div class="m-val-wrap"><span class="m-val">${d.fed}</span><span class="m-comp">(Prévu : ${d.fed_target})</span></div><div class="m-badge ${d.fed_status.cls}">${d.date_fed ? d.date_fed : d.fed_status.msg}</div></div>
                    <div class="m-row"><span class="m-lbl">CPI INFLATION</span><div class="m-val-wrap"><span class="m-val">${d.inf}</span><span class="m-comp">(Vs M-1 : ${d.inf_prev})</span></div><div class="m-badge ${d.inf_status.cls}">${d.date_cpi ? d.date_cpi : d.inf_status.msg}</div></div>
